...
import ControlTechniqueCreate from '.';
import { useAgsMutation } from '@ags/webclient-core/queries';
import {
    controlTechniqueCreateFormNavigateAndAssert,
...
describe('Create Control Technique Page', () => {
    ...
    test('render page and submit - failure scenario', async () => {
        mockMutateFn.mockImplementation((_request: any) => {
            useMutationOptions?.onError!(new Error('test error'), {
                name: fixtureCreateControlTechniqueRestType.name,
            });
        });

        const renderResult = render(
            <BrowserRouter>
                <ControlTechniqueCreate />
            </BrowserRouter>
        );
        ...
        expect(mockAddNotificationFn).toHaveBeenCalledWith({
            content: 'test error',
            dismissible: true,
            header: `Create Control Technique ${fixtureCreateControlTechniqueRestType.name} Failed.`,
    ...
    test('render page and submit - success scenario', async () => {
        ...
        expect(mockUseHistoryReplaceFn).toHaveBeenCalledWith('/controltechniques/123', {
            notifications: [
                {
                    dismissible: true,
                    header: `Create Control Technique ${fixtureCreateControlTechniqueRestType.name} Succeeded.`,
                    id: expect.any(String),
                    type: 'success',
                },
            ],
