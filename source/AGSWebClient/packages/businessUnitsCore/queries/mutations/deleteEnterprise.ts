...
export class DeleteEnterpriseMutation extends MutationBase<
...
> {
    ...
    async onComplete(
        ...
        if (error) {
            console.debug('Delete Enterprise onError ' + error);
        } else if (data) {
            console.debug('Delete Enterprise onSuccess ' + JSON.stringify(data));
            // invalidate cache when succeed
            this.queryClient.invalidateQueries(QueryType.LIST_BUSINESSUNITS);
        }
    }
}
