...
export class CreateEvidenceSchemaInputValidator extends InputValidator<CreateEvidenceSchemaInput> {
    protected validate(input: CreateEvidenceSchemaInput): void {
        if (!input.content) {
            this.errors.push('Schema content cannot be null or empty.');
        }

        if (!this.isValidJson(input.content)) {
            this.errors.push('Schema content must contain valid JSON.');
        }
